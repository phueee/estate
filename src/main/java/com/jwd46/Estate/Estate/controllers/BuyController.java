...
//    public String showBuy(@PathVariable("homeId") String homeIdStr, HttpSession session, Model model) {
//        int userId=(Integer) session.getAttribute("userId");
//        try {
...
//            // Handle the case where homeIdStr or userIdStr is not a valid integer
//            return "error";
//        }
//    }
    ...
    public String payment(@PathVariable ("homeId") int homeId,HttpSession session, RedirectAttributes redirectAttributes){
        Home home = homeDao.findByHomeId(homeId);
        int userId= (Integer) session.getAttribute("userId");
        User user=userDao.findByUserId(userId);
        UserHome userHome = new UserHome();
